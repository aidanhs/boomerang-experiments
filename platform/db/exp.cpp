...
Const::Const(int i)		: Exp(opIntConst),	conscript(0), type(new VoidType)
{
    u.i = i;
}
...
Const::Const(const char* p)	: Exp(opStrConst),	conscript(0), type(new VoidType)
{
    u.p = p;
}
...
void Ternary::print(std::ostream& os, bool html)
{
    ...
    switch (op)
        ...
        case opOpTable:
            ...
            if (p1) p1->print(os, html);
            else os << "<NULL>";
            os << ",";
            if (p2) p2->print(os, html);
            else os << "<NULL>";
            os << ",";
            if (p3) p3->print(os, html);
            else os << "<NULL>";
            os << ")";
            return;
        default:
    ...
    if (p1) p1->printr(os, html);
    else os << "<NULL>";
    if (op == opTern)
        {
            os << " ? ";
            if (p2) p2->printr(os, html);
            else os << "<NULL>";
