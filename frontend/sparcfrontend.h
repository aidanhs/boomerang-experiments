...
private:
	...
	void 	handleCall(UserProc *proc, ADDRESS dest, BasicBlock* callBB,
              Cfg* cfg, ADDRESS address, int offset = 0);
	...
	bool 	case_CALL(ADDRESS& address, DecodeResult& inst,
			    ...
			    UserProc* proc, std::set<CallStatement*>& callSet,
	...
	bool 	helperFuncLong(ADDRESS dest, ADDRESS addr, std::list<RTL*>* lrtl,
              std::string& name);
