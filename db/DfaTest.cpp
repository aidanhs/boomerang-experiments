...
void DfaTest::testMeetPointer() {
    ...
    bool ch = false;
    Type* res = pi32.meetWith(&pu32, ch);
    CPPUNIT_ASSERT(ch == true);
    std::ostringstream ost2;
    ost2 << res->getCtype();
    actual = ost2.str();
    ...
    ch = false;
    res = pi32.meetWith(&v, ch);
    CPPUNIT_ASSERT(ch == false);

    res = pi32.meetWith(&i32, ch);
    std::ostringstream ost3;
    ost3 << res;
    actual = ost3.str();
    expected = "union";
