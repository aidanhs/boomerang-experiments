                    ...
                    if (isBranch())
                        {
                            ...
                            // Trent: was the latter meant to be anything NOT subscripted?
                                    ...
                                    continue;
                                else if (VERBOSE) LOG << "Allowing prop. into branch (2) of " << def << "\n";
                        }
#endif
    ...
    if (condOp == opFlagCall && !strncmp(((Const*)pCond->getSubExp1())->getStr(), "SUBFLAGS", 8))
        {
            ...
            switch (jtCond)
                ...
                case BRANCH_JNE:
                    ...
                    break;
                case BRANCH_JSL:
                    if (makeUns) op = opLessUns;
                    else op = opLess;
                    break;
                case BRANCH_JSLE:
                    if (makeUns) op = opLessEqUns;
                    else op = opLessEq;
                    break;
                case BRANCH_JSGE:
                    if (makeUns) op = opGtrEqUns;
                    else op = opGtrEq;
                    break;
                case BRANCH_JSG:
                    if (makeUns) op = opGtrUns;
                    else op = opGtr;
                    break;
                case BRANCH_JUL:
                    op = opLessUns;
...
Assign::Assign(Assign& o) : Assignment(lhs->clone())
{
    kind = STMT_ASSIGN;
    rhs = o.rhs->clone();
    if (o.type)	 type  = o.type->clone();
    else type  = NULL;
    if (o.guard) guard = o.guard->clone();
    else guard = NULL;
}
...
bool	ImpRefStatement::processConstants(Prog* prog)
{
    return false;
}
